...
def plot_cluster_independent_telomeric_reads(mod_bam, telo_stats_fh, mod_table, mod_nucl, modification, file_name, summary_file, max_subtelo_stretch, c_strand_only, image_width=1500, image_height=1000):
    ...
    telo_table = pd.read_table(telo_stats_fh, delimiter="\t")
    telo_table["vrr_start_pos"] = telo_table["vrr_start_pos"].astype(int)
    telo_table["vrr_telo_length"] = telo_table["vrr_telo_length"].astype(int)
    telo_table["read_len"] = telo_table["read_len"].astype(int)

    telo_table["telo_end"] = telo_table["vrr_start_pos"] + telo_table["vrr_telo_length"]
    # telo_stats = {}
    ...
    telo_stats = {}
    for read in list(telo_table["read_id"]):
        telo_stats[read] = {"strand": telo_table.loc[telo_table["read_id"]==read, "strand"].item(),
                            "vrr_start_pos": telo_table.loc[telo_table["read_id"]==read, "vrr_start_pos"].item(),
                            "telo_end":  telo_table.loc[telo_table["read_id"]==read, "telo_end"].item(),
                            "read_len":telo_table.loc[telo_table["read_id"]==read, "read_len"].item(),
                            "vrr_telo_length":telo_table.loc[telo_table["read_id"]==read, "vrr_telo_length"].item() }
    ...
    for aln in mod_bam_fh:
        if aln.query_name in telo_stats and not aln.is_supplementary and not aln.is_secondary:
            ...
            telo_stats[aln.query_name]["read_len"] = len(aln.query_sequence)
            ...
            if telo_stats[aln.query_name]["strand"] == "G":
                ...
                telo_possible[aln.query_name] = {"full_read":telo_reads[aln.query_name].count(mod_nucl), 
                                      "subtelo":telo_reads[aln.query_name][0:telo_stats[aln.query_name]["vrr_start_pos"]].count(mod_nucl), 
                                      "telo":telo_reads[aln.query_name][telo_stats[aln.query_name]["vrr_start_pos"]:telo_stats[aln.query_name]["telo_end"]].count(mod_nucl)}
            else:
                telo_orientated[aln.query_name] = rev_comp(aln.query_sequence)
                telo_possible[aln.query_name] = {"full_read":telo_reads[aln.query_name].count(mod_nucl), 
                                      "subtelo":telo_reads[aln.query_name][int(telo_stats[aln.query_name]["read_len"])-int(telo_stats[aln.query_name]["vrr_start_pos"]):].count(mod_nucl), 
                                      "telo":telo_reads[aln.query_name][telo_stats[aln.query_name]["read_len"]-telo_stats[aln.query_name]["telo_end"]:telo_stats[aln.query_name]["read_len"]-telo_stats[aln.query_name]["vrr_start_pos"]].count(mod_nucl)}
    # load mod bam
    mod_dict = {}
            ...
            if line[13] == modification:          
                ...
                if valid_mod:
                    ...
                    if telo_stats[line[0]]["strand"] == "C":
                        mod_pos = telo_stats[line[0]]["read_len"] - int(line[1]) - 1
                    else:
                        mod_pos = int(line[1])
                    ...
                    if mod_pos > telo_stats[line[0]]["telo_end"]:
                        continue
                    elif mod_pos > telo_stats[line[0]]["vrr_start_pos"]:
    ...
    with open(summary_file, "w") as summary_fh:
        ...
        for read in telo_stats:
            if read not in mod_dict:
                summary_fh.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(read, telo_stats[read]["strand"], 
                    telo_stats[read]["vrr_telo_length"], "full_read", telo_possible[read]["full_read"], 0, 0))
                summary_fh.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(read, telo_stats[read]["strand"], 
                    telo_stats[read]["vrr_telo_length"], "subtelo", telo_possible[read]["subtelo"], 0, 0))
                summary_fh.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(read, telo_stats[read]["strand"], 
                    telo_stats[read]["vrr_telo_length"], "telo", telo_possible[read]["telo"], 0 ,0))
                continue
            if telo_possible[read]["telo"] == 0:
                summary_fh.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(read, telo_stats[read]["strand"], 
                    telo_stats[read]["vrr_telo_length"], "telo", 0, 0, 0))
            else:
                summary_fh.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(read, telo_stats[read]["strand"], 
                                                                telo_stats[read]["vrr_telo_length"], "telo", 
                                                                 telo_possible[read]["telo"], mod_dict[read]["telo"],
                                                                 mod_dict[read]["telo"]/telo_possible[read]["telo"] * 100))
            
            if telo_possible[read]["subtelo"] == 0:
                summary_fh.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(read, telo_stats[read]["strand"], 
                                                                        telo_stats[read]["vrr_telo_length"], "subtelo", 0, 0, 0))
            else:
                summary_fh.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(read, telo_stats[read]["strand"], 
                                                                    telo_stats[read]["vrr_telo_length"], "subtelo", 
            ...
            if telo_possible[read]["full_read"] == 0:
                summary_fh.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(read, telo_stats[read]["strand"], 
                                                                        telo_stats[read]["vrr_telo_length"], "full_read", 0, 0, 0))
            else:
                summary_fh.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(read, telo_stats[read]["strand"], 
                                                                    telo_stats[read]["vrr_telo_length"], "full_read", 
                                                                 telo_possible[read]["full_read"], mod_dict[read]["full_read"],
                                                                 mod_dict[read]["full_read"]/telo_possible[read]["full_read"] * 100))

            
            
    max_telo_length = max([telo_stats[i]["vrr_telo_length"] for i in telo_stats])
        ...
        for telo in sorted_telos:
            ...
            if telo in mod_dict:
                for pos in mod_dict[telo]["pos"]:
                    if pos < telo_stats[telo]["vrr_start_pos"] - subtelo_stretch_max:
                        continue
                    pos = pos - (telo_stats[telo]["vrr_start_pos"] - subtelo_stretch_max)
